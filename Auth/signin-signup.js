...
signupForm.addEventListener('submit', async (e) => {    // event listener for the signup form
    ...
    if(!firstname || !email || !username || !password || !confirm_password) {    // check all fields
        ...
        document.getElementById('error-message2').innerHTML = 'Please enter all the fields';
        console.log("Please enter all the fields")
        return;
    }
    if(!email.includes('@') || !email.includes('.')) {   // check email
        document.getElementById('error-message2').innerHTML = 'Please enter a valid email';
        console.log("Please enter all the fields")
        return;
    }
    // compare passwords  
    if(password != confirm_password) {
        document.getElementById('error-message2').innerHTML = 'The passwords you entered do not match';
        return;
    }
